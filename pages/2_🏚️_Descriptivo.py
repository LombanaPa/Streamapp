...
### filtro tipo
tipos = sorted(list(df['Tipo'].unique()))
tipos.append('todo')

tipo = st.sidebar.selectbox("Tipo Inmueble",tipos)
...
def Filtrandodataframe(df_fil,ciuda,pre_min,pre_max,estra,tipo_inmu):
    if tipo_inmu!="todo":
        return df_fil.loc[(df_fil['ciudad']==ciuda)&
                              (df_fil['Canon']>=pre_min)&
                              (df_fil['Canon']<=pre_max)&
                              (df_fil['Tipo']==tipo_inmu)&
                              (df_fil['ID_Estrato'].isin(estra))].\
...
if boton:
    df_filterin = Filtrandodataframe(df, ciudades, precio_min
                                     , precio_max,estrato,tipo) 
    df_filterin['Indice'] = df_filterin.index   
    col1,col2 = st.columns(2)
    if df_filterin.shape[0]==0:
        st.write("Por favor cambie los filtros, puede ser que algún filtro no arroje información")
    else:    
        with col1:
